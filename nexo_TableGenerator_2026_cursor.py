...
LICENSE = "License : Creative Commons CC BY-SA 4.0 https://creativecommons.org/licenses/by-sa/4.0/"
...
GENERATE_ISO_DESCRIPTION_FILE= False
DEFAULT_LOG_LEVEL=logging.INFO
LO_PATCH_LONG_NAME_SPLIT = True
...
def generate_message_table_from_schema(input_dir: str, xs: xmlschema.XMLSchema, output_dir: str) -> None:
    ...
    if 'Document' in xs.elements:
        ...
        message_title = xs.name
        v5_html: BeautifulSoup = BeautifulSoup()

        try:
            for elt in doc.iterchildren():
                message_title = xml_tags[elt.local_name][:-3] # type: ignore
                break
        ...
        if (COMPARE_TO_PREVIOUS_VERSION):
            try:
                previous_html = load_previous_version_html(message_title, "") # type: ignore
            except Exception as preve:
        ...
        if (SPECIAL_COMPARE_V5):
            try:
                v5_html = load_v5_version_html(
                    message_title, "") # type: ignore
            except Exception as preve:
                logger.info(f"Error loading v5 version HTML for {
                    message_title}: {preve}")
        # Generate table content
        html_table_head = generate_table_header(message_title) # type: ignore
        ...
        <table class="fixed" cellpadding="7" cellspacing="0" id="{head_component}" title="{head_component}" style="1px solid #a6a6a6; border-collapse: collapse;width: 840px;">
            <thead>
                <tr style="background:#f2f2f2;font-size:8pt;font-style: italic;font-weight: bold;vertical-align: top;border: 1px solid #a6a6a6;border-collapse: collapse;">
                    ...
                    <td style="font-size:10pt;border: 1px solid #a6a6a6;border-collapse: collapse;"><p lang="en-GB" class="western" style="text-align:center;color: #808080;">Lvl</p></td>
                    <td style="font-size:10pt;border: 1px solid #a6a6a6;border-collapse: collapse;"><p lang="en-GB" class="western" style="text-align:left;color: #808080;">{head_component}</p></td>
                    <td style="font-size:10pt;border: 1px solid #a6a6a6;border-collapse: collapse;"><p lang="en-GB" class="western" style="text-align:center;color: #808080;">Mult</p></td>
                    <td style="font-size:10pt;border: 1px solid #a6a6a6;border-collapse: collapse;"><p lang="en-GB" class="western" style="text-align:center;color: #808080;">Rule</p></td>
                    <td style="font-size:10pt;border: 1px solid #a6a6a6;border-collapse: collapse;"><p lang="en-GB" class="western" style="text-align:center;color: #808080;">Cstr</p></td>
                    <td style="font-size:10pt;border: 1px solid #a6a6a6;border-collapse: collapse;"><p lang="en-GB" class="western" style="text-align:left;color: #808080;">Usage</p></td>
                </tr>
            </thead>
    '''
...
def load_previous_version_html(message_name: str, element_type: str) -> BeautifulSoup:
    ...
    if (element_type == ''):
        ...
        if (element_type + ".html") in os.listdir(PREVIOUS_VERSION_DIR):
            # logger.debug("Found same version for " + element_type)
...
def previous_version_lookup(previous_version: BeautifulSoup, elt_name: str, elt_type: str, elt_mult: str) -> tuple[int, str, str, str]:
    ...
    if previous_version is not None:
        for row in previous_version.findAll(name="tr"):
            ...
            usage: str = re.sub('<br/>', ' ', unicodedata.normalize("NFKD", cells[5].text.strip()))
...
def process_schema_elements(xsd_element, level: int, previous_version: BeautifulSoup, v5_version: BeautifulSoup) -> str:
...
def get_element_info_XMLL2024(elt) -> dict:
    """
...
def clean_previous_usage(usage: str, elt_type: str) -> str:
    ...
    # Remove Intiial and ending line break if any
...
def generate_table_row(level: int, element_info: dict, mult_info: dict,
    ...
    #special ugly workaround for VERY LONG components names, and LibreOffice not being able to split words resulting incompressible table width
    # e.g. CardReaderApplicationProtocolDataUnitResponse
    #split at 34 characters with a newline \n, making the table compressible again

    element_name_patch = element_info['name']
    element_info_patch = element_info['type']
    if LO_PATCH_LONG_NAME_SPLIT:
        if len(element_info['name']) > 34:
            element_name_patch = element_info['name'][:34] + \
                "\n" + element_info['name'][34:]
        if len(element_info['name']) > 34:
            element_info_patch = element_info['type'][:34] + \
                "\n" + element_info['type'][34:]
    ...
    <tr id="{element_info['name']}.{element_info['type']}" class="" style="">
        <td style="color:{color};font-size:10pt;vertical-align:middle;text-align:left;border: 1px solid #a6a6a6;">{level}</font></td>
        <td style="color:{color};font-size:10pt;vertical-align:middle;text-align:left;border: 1px solid #a6a6a6;">{indent}{element_name_patch}</td>
        <td style="color:{color};font-size:10pt;vertical-align:middle;text-align:center;border: 1px solid #a6a6a6;">{mult_info['multiplicity']}</td>
        <td style="color:{color};font-size:10pt;vertical-align:middle;text-align:center;border: 1px solid #a6a6a6;">{prev_info['rule']}</td>
        <td style="color:{color};font-size:10pt;vertical-align:middle;text-align:center;border: 1px solid #a6a6a6;">{prev_info['constraint']}</td>
        <td style="color:{color};font-size:10pt;vertical-align:middle;text-align:left;border: 1px solid #a6a6a6;">{prev_info['usage']}<i>&lt;{element_info['tag']}&gt;:: {element_info_patch}</i></td>
    </tr>'''
...
def main() -> None:
    ...
    try:
        ...
        logger.info(f"PREVIOUS_VERSION_DIR: {PREVIOUS_VERSION_DIR}")
        logger.info(f"COMPARE_TO_PREVIOUS_VERSION: {
...
def cli_args(parser) -> None:
    parser.add_argument("-s", "--schema_dir", help="Path to the directory containing the XSD schemas. default: ./schema", default=SCHEMA_DIR, type=str)
    parser.add_argument("-o", "--output_dir", help="Path to the directory where the output HTML files will be saved. default: ./output_html", default=OUTPUT_DIR, type=str)

    parser.add_argument("-m", "--iso20022_model",
                        help="Path to the ISO20022 model file (download at https://www.iso20022.org/iso20022-repository/e-repository ).", default=ISO20022_MODEL, type=str)
    #parser.add_argument("-e", "--endpoints", help="Path to the endpoints file.", default=ENDPOINTS_FILE)
    parser.add_argument("-d", "--previous_version_dir",
                        help="Path to the directory containing the previous version HTML files. default: previous_html", default=PREVIOUS_VERSION_DIR, type=str)
    parser.add_argument("-r", "--repo_desc_file",
                        help=f"Path to the ISO repository description file. default: %iso_model_file%.description.json", default=REPO_DESC_FILE, type=str)
    parser.add_argument("-g", "--generate_endpoints", help="Generate endpoints.", default=GENERATE_ENDPOINTS,type=bool)
    parser.add_argument("-c", "--compare_previous", help="Compare with a previous versions. requires previous version HTML files. default: False",
                        default=ENDPOINT_COMPARE_TO_PREVIOUS_VERSION, type=bool)
    parser.add_argument("-e", "--endpoints_in_endpoint",
                        help="Stop at endpoints in endpoints, other endpoints contain the full component list. default: True", default=ENDPOINTS_IN_ENDPOINT, type=bool)
    parser.add_argument("-l", "--log_level", help="Set the logging level.", choices=['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'], default=DEFAULT_LOG_LEVEL)
    parser.add_argument("-i", "--iso_description_generate",
                        help="Generates a ISO Repository description file in the WORK DIR. default: False", action='store_true', default=False)
    parser.add_argument("-v", "--version", action="version", version="NEXO_MUG_TOOL Version: " + TOOL_VERSION)

def process_cli_args(parser:argparse.ArgumentParser):

    global SCHEMA_DIR, OUTPUT_DIR, ISO20022_MODEL, PREVIOUS_VERSION_DIR, REPO_DESC_FILE, GENERATE_ENDPOINTS, ENDPOINT_COMPARE_TO_PREVIOUS_VERSION, ENDPOINTS_IN_ENDPOINT, GENERATE_ISO_DESCRIPTION_FILE, logger
...
# Check if any arguments were provided
    if all(value is None for value in vars(args).values()):
        logger.info(
            "No options provided, using default options. Use -h for help.")
        return
    if args.schema_dir:
        SCHEMA_DIR = args.schema_dir
    if args.output_dir:
        OUTPUT_DIR = args.output_dir
    if args.iso20022_model:
        ISO20022_MODEL = args.iso20022_model
    if args.previous_version_dir:
        PREVIOUS_VERSION_DIR = args.previous_version_dir
    if args.repo_desc_file:
        REPO_DESC_FILE = args.repo_desc_file
    if args.generate_endpoints:
        GENERATE_ENDPOINTS = args.generate_endpoints
    if args.compare_previous:
        ENDPOINT_COMPARE_TO_PREVIOUS_VERSION = args.compare_previous
    if args.endpoints_in_endpoint:
        ENDPOINTS_IN_ENDPOINT = args.endpoints_in_endpoint
    if args.iso_description_generate:
        GENERATE_ISO_DESCRIPTION_FILE = args.iso_description_generate
