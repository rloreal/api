...
OUTPUT_DIR = "C:/Users/romai/OneDrive/Documents/NEXO/api/casp_v5/NEW_HTML/"
...
def collect_all_endpoints_elements(doc, endpoints_dict:dict[str, xmlschema.validators.elements.XsdElement]) -> dict[str, xmlschema.validators.elements.XsdElement]:
    ...
    Args:
        xs : a XMLSchema Element document root
    Returns:
         a dictionary mapping element types to XsdElement objects for endpoints.
    """
...
def generate_message_table_from_schema(input_dir: str, xs: xmlschema.XMLSchema, output_dir: str, endpoints_elements:dict) -> None:
    ...
    Args:
        ...
        endpoints_elements: dictionary mapping element types to XsdElement objects for endpoints.
...
def process_schema(xs: xmlschema.XMLSchema, endpoint_elts: dict[str, xmlschema.XsdElement]):
    ...
    generate_message_table_from_schema(
        SCHEMA_DIR, xs, OUTPUT_DIR, endpoint_elts)  # type: ignore
            ...
            try:
                if (ENDPOINT_COMPARE_TO_PREVIOUS_VERSION):
                    bs_previous_html = load_previous_version_html(
                        xs.name, element_type) # type: ignore
            except Exception as ex:
                logger.error(f"Error loading previous version HTML for {
                    element_type}: {ex}")
...
def main() -> None:
    ...
    try:
        ...
        logger.info(f"An error occurred: {ge}")
        traceback.print_exc()
